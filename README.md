...
Then set these environment variables. Add them to `~/.bashrc` if you want them in every new terminal.
```bash
...
3. Build the main project. CMake also builds `xspublic` for the Xsens MTi sensors.
```bash
cd ~/humanoid_controller
...
Make sure the Pinocchio environment variables from the Build section are still set.
...
## Troubleshooting (Jetson Orin Nano)
...
The `vermagic` string must match your running kernel. Check your kernel with `uname -r`.
...
Use the same interface name as in your motor config.
For example, `config/hardware/motor/olaf.yaml` uses `can0`.
